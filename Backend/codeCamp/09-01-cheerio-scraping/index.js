...
async function createBoardAPI(mydata) {
  ...
  const $ = cheerio.load(result.data); // html에서 받아온 문서 cheerio에 로드
  $('meta').each((_,el)=>{
      if($(el).attr('property')) {
          const key = $(el).attr('property').split(':')[1] // og:url, og:type, og:title 등등
...
const frontendData = {
    title: "안녕하세요~~~",
    contents: "여기 정말 좋은거 같아요! 한번 꼭 놀러오세요!! 여기가 어디냐면 https://www.naver.com 이에요!!!"
}
